...
class PlanningState(str, enum.Enum):
    """Explicit states of the Phase 2 planning state machine."""
...
class ProposedPlan(BaseModel):
    """Structured plan proposal generated by the Planner agent before verification."""

    plan_id: UUID = Field(default_factory=uuid4)
    incident_id: UUID | None = Field(default=None)
    objective: str = Field(default="", description="Operational goal of the plan.")
    reason: str = Field(default="", description="Planner rationale.")
    steps: list[PlanStep] = Field(default_factory=list)
    restrictions_considered: list[str] = Field(default_factory=list)
    planner_version: str = Field(default="mock-planner-v1")
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )
...
class VerificationResult(BaseModel):
    """Structured output produced by the Verifier agent."""

    approved: bool = Field(description="Whether the proposed plan passed verification.")
    risk_level: str = Field(default="low", description="Assessed risk level ('low', 'medium', 'high', 'critical').")
    issues: list[str] = Field(default_factory=list, description="Critical errors or prohibited steps identified.")
    warnings: list[str] = Field(default_factory=list, description="Non-blocking observations or recommendations.")
    checked_restrictions: list[str] = Field(default_factory=list, description="List of restrictions validated.")
    verifier_version: str = Field(default="mock-verifier-v1")
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )
